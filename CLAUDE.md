...
**The motion lease is THE safety mechanism — there is no keep-alive.** The app never sends press/release events or heartbeats; it streams the **absolute drive state** (`dv<throttle><steer>`) — immediately on every change, re-asserted every 150ms while any control is engaged, and once a second when idle (that idle tick is not a safety signal; it only keeps the phone's Wi-Fi radio out of power-save through driving pauses). The firmware honours a **non-neutral throttle for only ~600ms** since the last `dv` frame (`motionLeaseMs`, which must match `MOTION_LEASE_MS` in `shared/protocol.ts`), then coasts to neutral and waits for fresh input. Consequences worth internalising before touching any of it: a lost frame is self-corrected by the next refresh (no stuck-throttle class of bug); a dead app, dead phone, dead Wi-Fi, dead bridge and dead radio all degrade identically (frames stop → car coasts ≤600ms); a parked car needs no traffic at all, so pause-time link blackouts are harmless by construction; and **nothing anywhere is allowed to generate or extend the lease on the app's behalf** — the bridge is a dumb relay whose only safety action is one event-driven `st` when the last app socket actually closes (plus on shutdown). The app side lives in `src/utils/drive-state.ts` (the drive screen's session: state + ticker + AppState background stop) and `src/utils/drive-locks.ts` → `DriveSessionModule.kt` (Android WifiLock `WIFI_MODE_FULL_LOW_LATENCY` + keep-screen-on for the session; best-effort, never a safety dependency).
