...
> Reconciled with the firmware (`feature/arduino`): the 2018 accelerometer drive mode (`dm`/`ad`/`as`) and the never-implemented `cm` camera + phantom `ab` codes were removed from the protocol, the firmware, and the simulator — the app drives via the on-screen throttle touchpad (forward/reverse) and the two steering buttons. `rc` (range-servo trim) gained an app screen (see Range Calibrate below); it had been firmware-only. The 2026 motion-lease redesign then removed the edge-triggered `db` press/release events (a lost release frame could leave the throttle stuck) and the `kp` keep-alive (liveness inference that false-tripped on Wi-Fi jitter AND kept a stale throttle alive) in favour of the absolute `dv` drive state.
...
**Utils are TS modules** (`src/utils/*.ts`) built on the shared protocol: `transmitter.send()`, `receiver.receive()`, `settings.sendAll()`, `drive-state.startDriveSession/setThrottle/setSteer()`, `websocket.createSocket/getSocket()`, `vibrate()`. Each also keeps a backward-compatible default export (e.g. `Transmitter.send`) so the older `.js` presentational components keep working unchanged.
...
**The motion lease is THE safety mechanism — there is no keep-alive.** The app never sends press/release events or heartbeats; it streams the **absolute drive state** (`dv<throttle><steer>[level]`) — immediately on every change, re-asserted every 150ms while any control is engaged, and once a second when idle (that idle tick is not a safety signal; it only keeps the phone's Wi-Fi radio out of power-save through driving pauses). Forward frames carry the 0..100 magnitude from the touchpad; neutral and reverse do not. A forward level below one step (5%) coerces to neutral so a resting finger holds no lease. The firmware honours a **non-neutral throttle for only ~400ms** since the last `dv` frame (`motionLeaseMs`, which must match `MOTION_LEASE_MS` in `shared/protocol.ts`), then — if the car was rolling forward — actively **brakes to a full stop** (brake-to-stop) rather than freewheeling into something, and waits for fresh input. A *deliberate* stop (leaving the drive screen / backgrounding / the bridge seeing the last socket close → `st`) and the motor-temp cutoff still coast to neutral via `stopCar()`; only a lost link mid-drive brakes. The firmware maps the level linearly onto the ESC servo angle (90° idle to `speedFactor`°), so the Speed (`sf`) screen is now the max-speed ceiling — a lower speed setting makes even a full-slider push gentle. Consequences worth internalising before touching any of it: a lost frame is self-corrected by the next refresh (no stuck-throttle class of bug); a dead app, dead phone, dead Wi-Fi, dead bridge and dead radio all degrade identically (frames stop → the lease expires within ~400ms → a forward-moving car brakes to a stop); a parked car needs no traffic at all, so pause-time link blackouts are harmless by construction; and **nothing anywhere is allowed to generate or extend the lease on the app's behalf** — the bridge is a dumb relay whose only safety action is one event-driven `st` when the last app socket actually closes (plus on shutdown). The app side lives in `src/utils/drive-state.ts` (the drive screen's session: state + ticker + AppState background stop) and `src/utils/drive-locks.ts` → `DriveSessionModule.kt` (Android WifiLock `WIFI_MODE_FULL_LOW_LATENCY` + keep-screen-on for the session; best-effort, never a safety dependency).
...
**Drive controls — throttle touchpad.** The app's forward/reverse buttons were replaced by a left-side vertical touchpad (`ThrottlePad`): top three-quarters gives variable forward (0..100%), bottom quarter gives fixed reverse, and the boundary is neutral. The touchpad springs back to neutral on release. Finger position maps through an app-side expo curve (`src/utils/throttle-curve.ts`, `THROTTLE_GAMMA`) and quantizes to 5% steps (DRIVE_LEVEL_STEP); same-direction level changes coalesce for efficiency (`SEND_MIN_INTERVAL_MS` in `src/utils/drive-state.ts`), and the existing 150ms heartbeat re-asserts the state. Steering is still the two react-native-gesture-handler buttons (`DriveButton`) — they use RNGH, not `Pressable`/`Touchable`, so the throttle and steer can be held simultaneously. Press fires on `onBegin`, release on `onFinalize` (RNGH guarantees they pair, so neither can stick on). `GestureHandlerRootView` wraps the app in `App.tsx`, and `index.js` imports `react-native-gesture-handler` first. See [ANDROID_UPGRADE.md](ANDROID_UPGRADE.md) §8.
