...
> Reconciled with the firmware (`feature/arduino`): the 2018 accelerometer drive mode (`dm`/`ad`/`as`) and the never-implemented `cm` camera + phantom `ab` codes were removed from the protocol, the firmware, and the simulator — the app drives only via the on-screen buttons. `rc` (range-servo trim) gained an app screen (see Range Calibrate below); it had been firmware-only. The 2026 motion-lease redesign then removed the edge-triggered `db` press/release events (a lost release frame could leave the throttle stuck) and the `kp` keep-alive (liveness inference that false-tripped on Wi-Fi jitter AND kept a stale throttle alive) in favour of the absolute `dv` drive state.
